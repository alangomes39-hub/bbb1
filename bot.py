...
⚠️ IMPORTANT:
Access released ONLY on 01/02/2026.
"""
...
PRODUCTS = {
    ...
    "buy_philippines": ("Filipinas", False),
...
async def set_language(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(name.replace("_", " ").title(), callback_data=key)]
...
async def notify_2026(context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, lang, product in buyers:
        await context.bot.send_message(
            uid,
            f"🎉 {product} liberado! Fale com o suporte."
            if lang == "pt"
            else f"🎉 {product} is live! Contact support."
        )
